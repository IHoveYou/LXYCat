...
public class TanyPhotoUtil {
    ...
    public static Uri createImagePathUri(final Context context) {
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            ToastUtils.showMessage("请先获取写入SDCard权限");
        } else {
...
//            Cursor cursor = context.getContentResolver().query(uri, new String[]{MediaStore.Images.ImageColumns.DATA}, null, null, null);
    ...
    public static String getImageAbsolutePath(Context context, Uri imageUri) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT && DocumentsContract.isDocumentUri(context, imageUri)) {
            if (TanyFileUtil.isExternalStorageDocument(imageUri)) {
            ...
            } else if (TanyFileUtil.isDownloadsDocument(imageUri)) {
                String id = DocumentsContract.getDocumentId(imageUri);
                Uri contentUri = ContentUris.withAppendedId(Uri.parse("content://downloads/public_downloads"), Long.valueOf(id));
                return TanyFileUtil.getDataColumn(context, contentUri, null, null);
            } else if (TanyFileUtil.isMediaDocument(imageUri)) {
                String docId = DocumentsContract.getDocumentId(imageUri);
                String[] split = docId.split(":");
                String type = split[0];
                Uri contentUri = null;
                ...
                return TanyFileUtil.getDataColumn(context, contentUri, selection, selectionArgs);
            }
        } // MediaStore (and general)
        else if ("content".equalsIgnoreCase(imageUri.getScheme())) {
            // Return the remote address
            if (TanyFileUtil.isGooglePhotosUri(imageUri))
                return imageUri.getLastPathSegment();
            return TanyFileUtil.getDataColumn(context, imageUri, null, null);
        }
        // File
