...
import com.tany.base.listener.OnDoIntListener;
...
public class TanyAnimationUtil {
...
//                textView.setTextColor((Integer) animation.getAnimatedValue());
    ...
    public static void cardFilpAnimation(final View beforeView, final View AfterView) {
        ...
        if (beforeView.getVisibility() == View.GONE) {
            ...
            TanyImageUtil.invisToVis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", -90f, 0f);
            TanyImageUtil.visToInvis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", 0f, 90f);
        } else if (AfterView.getVisibility() == View.GONE) {
            TanyImageUtil.invisToVis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", -90f, 0f);
            TanyImageUtil.visToInvis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", 0f, 90f);
        }

        TanyImageUtil.visToInvis.setDuration(250);// 翻转速度
        TanyImageUtil.visToInvis.setInterpolator(accelerator);// 在动画开始的地方速率改变比较慢，然后开始加速
        TanyImageUtil.invisToVis.setDuration(250);
        TanyImageUtil.invisToVis.setInterpolator(decelerator);
        TanyImageUtil.visToInvis.addListener(new Animator.AnimatorListener() {

            @Override
            public void onAnimationEnd(Animator arg0) {
                if (beforeView.getVisibility() == View.GONE) {
                    AfterView.setVisibility(View.GONE);
                    TanyImageUtil.invisToVis.start();
                    beforeView.setVisibility(View.VISIBLE);
                } else {
                    AfterView.setVisibility(View.GONE);
                    TanyImageUtil.visToInvis.start();
                    beforeView.setVisibility(View.VISIBLE);
                }
            }
