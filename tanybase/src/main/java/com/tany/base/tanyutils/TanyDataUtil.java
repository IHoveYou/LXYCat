...
public class TanyDataUtil {
    ...
    public static int[] stringToInts(String s) {
        ...
        if (TanyDataUtil.isNullString(s)) {
     ...
     */
    public static double byte2Size(long byteNum, TanyConstUtil.MemoryUnit unit) {
        if (byteNum < 0) {
            return -1;
        }
        switch (unit) {
            default:
            case BYTE:
                return (double) byteNum / TanyConstUtil.BYTE;
            case KB:
                return (double) byteNum / TanyConstUtil.KB;
            case MB:
                return (double) byteNum / TanyConstUtil.MB;
            case GB:
                return (double) byteNum / TanyConstUtil.GB;
        }
    }
     ...
     * @return 字节数
     */
    public static long size2Byte(long size, TanyConstUtil.MemoryUnit unit) {
        if (size < 0) {
            return -1;
        }
        switch (unit) {
            default:
            case BYTE:
                return size * TanyConstUtil.BYTE;
            case KB:
                return size * TanyConstUtil.KB;
            case MB:
                return size * TanyConstUtil.MB;
            case GB:
                return size * TanyConstUtil.GB;
        }
    }
    ...
    public static String byte2FitSize(long byteNum) {
        ...
        } else if (byteNum < TanyConstUtil.KB) {
            return String.format(Locale.getDefault(), "%.3fB", (double) byteNum);
        } else if (byteNum < TanyConstUtil.MB) {
            return String.format(Locale.getDefault(), "%.3fKB", (double) byteNum / TanyConstUtil.KB);
        } else if (byteNum < TanyConstUtil.GB) {
            return String.format(Locale.getDefault(), "%.3fMB", (double) byteNum / TanyConstUtil.MB);
        } else {
            return String.format(Locale.getDefault(), "%.3fGB", (double) byteNum / TanyConstUtil.GB);
        }
    }
    ...
    public static ByteArrayOutputStream input2OutputStream(InputStream is) {
        ...
        try {
            ...
            byte[] b = new byte[TanyConstUtil.KB];
            int len;
            while ((len = is.read(b, 0, TanyConstUtil.KB)) != -1) {
                os.write(b, 0, len);
            }
            return os;
        } catch (IOException e) {
